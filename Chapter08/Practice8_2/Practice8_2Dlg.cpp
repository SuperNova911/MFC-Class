...
void CPractice8_2Dlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	else
	{
		...
		strSender.Format(TEXT("송신측 주소 : %d.%d.%d.%d\t\t"), b1, b2, b3, b4);
		
		m_addrReceiverIP.GetAddress(b1, b2, b3, b4);
		strReceiver.Format(TEXT("수신측 주소 : %d.%d.%d.%d\t\t"), b1, b2, b3, b4);

		CTime timeTime;
		DWORD dwResult = m_timeTransmit.GetTime(timeTime);

		CString strDate;
		strDate.Format(TEXT("전송날짜 : %s\r\n\r\n"), timeTime.Format("%A, %B, %d, %y"));
