...
//extern "C" _declspec(dllimport) void Calender(int year, int month, int *Days);
...
void CPractice11_1Dlg::OnPaint()
{
	...
	else
	{
		...
		for (i = 0; i < 7; i++)
		{
			dc.TextOutW(x, y, m_nWeekDays[i]);
			x += 35;
		}

		//Calender(m_nYear, m_nMonth, &m_nDays[0][0]);
		HINSTANCE hDll;
		hDll = LoadLibrary(TEXT("CalenderDLL.dll"));

		typedef int(*CalenderFunc) (int year, int month, int *Days);

		CalenderFunc lpCalender;

		lpCalender = (CalenderFunc)GetProcAddress(hDll, "Canlender");
