...
void CPractice11_3Dlg::OnClickedButtonCalcRoot()
{
	...
	if (fA == 0)
	{
		AfxMessageBox(TEXT("첫 번째 계수는 0이 될 수 없습니다."));
		SetDlgItemText(IDC_EDIT_A, TEXT(""));
		SetDlgItemText(IDC_EDIT_B, TEXT(""));
		SetDlgItemText(IDC_EDIT_C, TEXT(""));
	}
	else
	{
		...
		int nDet = m_calcRoot.Derterminant(fA, fB, fC, &fDet);

		strTemp.Format(TEXT("%g"), fDet);

		switch (nDet)
		{
		case 0:
			SetDlgItemText(IDC_EDIT_RESULT, TEXT("근이 없다"));
			SetDlgItemText(IDC_EDIT_DETERMINANT, strTemp);
			SetDlgItemText(IDC_EDIT_ROOT1, TEXT(""));
			SetDlgItemText(IDC_EDIT_ROOT2, TEXT(""));
			break;

		case 1:
			SetDlgItemText(IDC_EDIT_RESULT, TEXT("중근"));
			SetDlgItemText(IDC_EDIT_DETERMINANT, strTemp);
			strTemp.Format(TEXT("%g"), m_calcRoot.CalcRoot1(fA, fB, fC));
			SetDlgItemText(IDC_EDIT_ROOT1, strTemp);
			strTemp.Format(TEXT("%g"), m_calcRoot.CalcRoot2(fA, fB, fC));
			SetDlgItemText(IDC_EDIT_ROOT2, strTemp);
			break;

		case 2:
			SetDlgItemText(IDC_EDIT_RESULT, TEXT("2개의 근이 존재"));
			SetDlgItemText(IDC_EDIT_DETERMINANT, strTemp);
			strTemp.Format(TEXT("%g"), m_calcRoot.CalcRoot1(fA, fB, fC));
			SetDlgItemText(IDC_EDIT_ROOT1, strTemp);
			strTemp.Format(TEXT("%g"), m_calcRoot.CalcRoot2(fA, fB, fC));
...
void CPractice11_3Dlg::OnClickedButtonClear()
{
	...
	SetDlgItemText(IDC_EDIT_B, TEXT(""));
	SetDlgItemText(IDC_EDIT_C, TEXT(""));
	SetDlgItemText(IDC_EDIT_RESULT, TEXT(""));
	SetDlgItemText(IDC_EDIT_DETERMINANT, TEXT(""));
	SetDlgItemText(IDC_EDIT_ROOT1, TEXT(""));
	SetDlgItemText(IDC_EDIT_ROOT2, TEXT(""));
}
