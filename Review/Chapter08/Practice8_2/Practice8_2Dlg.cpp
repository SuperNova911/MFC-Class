...
void CPractice8_2Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//  DDX_Control(pDX, IDC_IPADDRESS_RECEIVER, addrReceiverIP);
...
void CPractice8_2Dlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (m_nTransmitRate != 100)
	{
		...
		strSummary.Format(TEXT("데이터 전송 중입니다 - %d 퍼센트 \r\n\r\n 잠시 기다려주십시오......"), m_nTransmitRate);
	...
	else
	{
		...
		m_prgsTransmit.SetPos(m_nTransmitRate);
		strTrans.Format(TEXT("데이터 전송을 완료했습니다. - %d 퍼센트\r\n\r\n"), m_nTransmitRate);

		BYTE b1, b2, b3, b4;

		CString strSender, strReceiver;
		m_addrSenderIP.GetAddress(b1, b2, b3, b4);
		strSender.Format(TEXT("송신측 주소 : %d.%d.%d.%d\r\n\r\n"), b1, b2, b3, b4);

		m_addrReceiverIP.GetAddress(b1, b2, b3, b4);
		strReceiver.Format(TEXT("수신측 주소 : %d.%d.%d.%d\r\n\r\n"), b1, b2, b3, b4);

		CTime timeTime;
		DWORD dwResult = m_timeTransmit.GetTime(timeTime);

		CString strDate;
		strDate.Format(TEXT("전송 날짜 : %s\r\n\r\n"), timeTime.Format("%A, %B, %d, %Y"));

		CString strData;
		strData.Format(TEXT("전송 데이터 내용 : %d"), m_nData);
