...
{
	if (m_nViewFlag == SECOND_VIEW)
	{
		FirstView();
		m_nViewFlag = FIRST_VIEW;
	}
	else if (m_nViewFlag == FIRST_VIEW)
	{
		SecondView();
		m_nViewFlag = SECOND_VIEW;
	}
...
void CMainFrame::FirstView()
{
	...
	m_pSecondView->ShowWindow(SW_HIDE);
...
{
	m_pFirstView = (CPractice10_3View*)GetActiveView();

	if (m_pSecondView == NULL)
	{
		m_pSecondView = new CSecondView;
		CCreateContext context;

		context.m_pCurrentDoc = m_pFirstView->GetDocument();
		m_pSecondView->Create(NULL, NULL, 0L, CFrameWnd::rectDefault, this, 2, &context);
