...
{
	CDialogEx::DoDataExchange(pDX);
	//  DDX_Control(pDX, IDC_EDIT_YEAR, m_nYear);
...
void CPractice11_1Dlg::OnPaint()
{
	...
	else
	{
		...
		CString m_WeekDays[7] = { TEXT("Sun"), TEXT("Mon"), TEXT("Tue"), TEXT("Wed"), TEXT("Thu"), TEXT("Fri"), TEXT("Sat") };
		CString m_Number;
		...
		HINSTANCE hDll;
		hDll = LoadLibrary(TEXT("CalendarDll.dll"));

		typedef int(*CalendarFunc)(int year, int month, int *Days);

		CalendarFunc lpCalendar;
		lpCalendar = (CalendarFunc)GetProcAddress(hDll, "Calendar");
		lpCalendar(m_nYear, m_nMonth, &m_nDays[0][0]);
		...
		for (i = 0; i < 6; i++)
		{
			...
			for (j = 0; j < 7; j++)
			{
				if (m_nDays[i][j] != 0)
				{
					m_Number.Format(TEXT("%d"), m_nDays[i][j]);
